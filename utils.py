...
def transform(image, boxes, labels, difficulties, split):
    ...
    new_image = image # 将uint8的numpy [h,w,96]
    new_boxes = boxes
    new_labels = labels
    new_difficulties = difficulties
    
    new_image = torch.from_numpy(new_image).permute(2,0,1).float()/255. # [0,1] [96,h,w] tensor
    ...
    new_image, new_boxes = resize(new_image, new_boxes, dims=(300, 300)) # 送入随机crop过的，[96,h,w]的tensor,返回[96,300,300]的tensor
    # Convert PIL image to Torch tensor
    #new_image = FT.to_tensor(new_image)
